...
import { Trophy, CalendarDays, Activity, ListOrdered, ClipboardList, Users, BarChart3, Settings, Clock, ArrowRight } from "lucide-react";
...
export default function Home() {
  ...
  const tournamentDate = new Date(2026, 5, 20); // June 20, 2026
        ...
        <div className="relative p-6 md:p-10 flex flex-col items-center text-center space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm md:text-base font-medium text-muted-foreground mt-4">
            ...
            <span className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-full border">
              ...
              {daysUntil > 0 ? `${daysUntil} days to go` : "Tournament Day!"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-2">
            ...
            <div className="text-3xl font-black">{statsLoading ? "-" : stats?.totalMatches ? 5 : 5}</div>
